...
# force cleanup any temporary data that is used later now, to prevent any issues caused by garbage in from the system running the command (env vars used here already being set/used unintentionally in the shell running the command, or temp files left over from previous halted executions that weren't cleaned, etc.)
scriptpath=''
newcommits=''
pullupdates=''
recompile=''
...
for cmd in git find notify-send realpath dirname pwd grep pkexec; do # check that the commands/utilities that the script uses actually exist, before doing anything.
...
# sanity checking execution location and PWD, also finds and changes the PWD to where the script actually lives on disk. **should** prevent any issues.
...
if [ -z $TEMPDEBUG ]; then # debug feature, so that I don't spam github/gitlab/etc while testing parts of the script that don't need to interact with the git repos or the git CLI to work.
	...
	for i in `find -L . -maxdepth 5 -type d -name ".git"`; do # for each .git directory found residing in any level of subdirectory including following symlinks below the folder the script lives in...
		...
		git remote update # update references to remote, do not output to console
		...
		if [ $(grep -c "Your branch is behind" $scriptpath/tempfilestatus) = "1" -a "$recompile" = "true" ]; then
	...
	if [ -s $scriptpath/tempfileneedsupdate ]; then # if the previous for loop found any repos with new commits, and...
		if [[ $pullupdates -eq '' ]]; then # updates are not to be pulled, then...
